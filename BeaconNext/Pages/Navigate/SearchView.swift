...
struct SearchView: View {
    ...
    private func iconColor(for typeCode: String?) -> Color {
            ...
            if !searchManager.lastSearchResults.isEmpty {
                List(searchManager.lastSearchResults, id: \.uid) { poi in
                    HStack(alignment: .top, spacing: 16) {
                        Image(systemName: iconName(for: poi.typecode))
                            .font(.system(size: 36))
                            .foregroundColor(iconColor(for: poi.typecode))
                            ...
                            if let address = poi.address {
                                if let geoPoint = poi.location,
                                   let dist = calculateDistance(to: geoPoint) {
                                    Text("\(formattedDistance(dist)) · \(address)")
                                        .font(.subheadline)
                                        .foregroundColor(.secondary)
                                        .lineLimit(1)
                                        .truncationMode(.tail)
